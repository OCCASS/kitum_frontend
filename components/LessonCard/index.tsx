...
function LessonCardStatusBar({lesson}: { lesson: ILesson }) {
    ...
    if (lesson.isClosed) return <p className={`${defaultClassName} text-gray-500/50`}><XMarkIcon
        className="size-5"/> Урок закрыт</p>
    if (lesson.status === "tasks_completed") return <p className={`${defaultClassName} text-green`}><CheckBadgeIcon
        className="size-5"/> Урок пройден</p>
}
            ...
            <div className="space-y-2">
                {lesson.status === "completed" ?
                    ...
                    <LinkButton href={`/lessons/${lesson.id}`}
                                className={`${disabled && "bg-gray-400/50 text-gray-600"}`}>Перейти</LinkButton>
                }
